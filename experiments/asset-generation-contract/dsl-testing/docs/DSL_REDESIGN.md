...
This is also a valid minimal scene. The compiler supplies the technical defaults selected by the project profile and emits a root scene contract. The author adds cast, spatial structure, exact ranges, and specialized validation only when those details matter to the direction.
...
| `thing` | A reusable prop, creature, character, material family, or asset identity |
...
## Adding cast and spatial direction

The author introduces entities only for subjects that matter creatively. Local `cast` keys provide stable, readable identity without repeating long IDs.
...
The compiler derives scoped IDs from `blue-winter-grove`, `clearing`, and `oldGrowth`. The author passes `clearing` and `oldGrowth` as values, so there is no `ref('region.clearing.primary')` bookkeeping and no unresolved string reference after a normal refactor.
...
| `thing` | prototype entity, bounds, form grammar, material roles, and variation components |
...
- Entries in `cast`, `worlds`, `scenes`, and similar records use their property key as scoped identity.
- Reusable imported definitions carry their own contract key.
...
2. Add named cast only for subjects that need control or traceability.
...
## The first implementation should change

Do not implement [GOAL.md](../GOAL.md) unchanged. Its compiler and validation requirements remain useful, but its required public authoring API proves the wrong surface.

The first vertical slice should implement:

1. `scene`, `thing`, `region`, and `population` authoring types.
2. `experience`, `visual`, `gameplay`, `rules`, `variation`, and `acceptance` semantic components.
3. Scoped identity through root keys and `cast` record keys.
4. Direct typed references by passing definition values.
5. One `frames` relationship helper.
6. One named project profile for technical defaults.
7. Deterministic lowerers into the supplied scene schema and component catalog.
8. A compact Blue Winter Grove TypeScript fixture under 120 lines.
9. Type-level, runtime, schema, reference, and deterministic-output tests.
10. Validation of the existing full JSON fixture without modifying it.
...
- The compact fixture is under 120 lines and contains no direct `component(...)`, `defineSystem`, raw catalog component names, system phases, camera vectors, schema versions, or long engine IDs.
- Experience, visual direction, gameplay direction, cast, composition, variation, rules, and acceptance are discoverable through autocomplete.
...
- Record keys provide stable scoped identity.
