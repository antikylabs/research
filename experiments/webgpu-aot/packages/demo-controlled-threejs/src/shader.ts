export const CONTROLLED_THREE_SCENE_WGSL = /* wgsl */ `
...
fn controlledThreeShade(normal: vec3f, view: vec3f, light: vec3f, albedo: vec3f, roughness: f32) -> vec3f {
  ...
  let geometry = geometrySchlickGgx(nDotL, roughness) * geometrySchlickGgx(nDotV, roughness);
  let specular = fresnel * distribution * geometry / max(4.0 * nDotL * nDotV, 0.0001);
  let diffuse = (vec3f(1.0) - fresnel) * albedo / 3.14159265;
  return (diffuse + specular) * nDotL;
}

fn controlledThreeScene(uv: vec2f) -> vec4f {
  ...
  let roughness = 0.28 + 0.55 * uv.y;
  let albedo = mix(vec3f(0.04, 0.16, 0.24), vec3f(0.82, 0.18, 0.06), uv.x);
  ...
  for (var lightIndex = 0; lightIndex < 32; lightIndex += 1) {
    ...
    color += controlledThreeShade(normal, view, light, albedo, roughness) * tint * 0.34;
  }
...
export const CONTROLLED_THREE_COMPOSITE_WGSL = /* wgsl */ `
fn controlledThreeComposite(color: vec3f) -> vec4f {
  let mapped = clamp(
