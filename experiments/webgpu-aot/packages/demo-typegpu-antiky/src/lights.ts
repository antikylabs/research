import {
  ANTIKY_ANIMATED_LIGHT_COUNT,
  ANTIKY_BILLBOARD_COUNT,
  ANTIKY_CURVE_LIGHT_COUNT,
  ANTIKY_FIRE_LIGHT_COUNT,
  ANTIKY_FIXED_TIME_STEP_SECONDS,
  createAntikyLightSimulation,
  type AntikyLightSimulation,
} from "./light-state.js";

export const ANTIKY_FIRE_PARTICLE_COUNT = ANTIKY_FIRE_LIGHT_COUNT;
export const ANTIKY_CURVE_PARTICLE_COUNT = ANTIKY_CURVE_LIGHT_COUNT;
export const ANTIKY_PARTICLE_COUNT = ANTIKY_BILLBOARD_COUNT;
export const ANTIKY_ANALYTIC_LIGHT_COUNT = 32;
export const ANTIKY_ANALYTIC_LIGHT_CAPACITY = 64;
export const ANTIKY_SIMULATED_LIGHT_COUNT = ANTIKY_ANIMATED_LIGHT_COUNT + 4;
...
export const ANTIKY_ANALYTIC_SOURCE_INDICES = [
  0, 16, 32, 48, 64, 80, 96, 112, 128, 144, 160, 176, 192, 208, 224,
  240, 256, 274, 293, 312, 331, 349, 368, 387, 406, 422, 438, 454,
] as const;

export interface AntikyLightRig {
...
function packParticleValues(
  ...
  simulation: AntikyLightSimulation,
): void {
  const { seeds, state } = simulation;
  for (let index = 0; index < ANTIKY_PARTICLE_COUNT; index += 1) {
...
function packLightValues(
  ...
  for (
    let sample = 0;
    sample < ANTIKY_ANALYTIC_SOURCE_INDICES.length;
    sample += 1
  ) {
    const sourceIndex = ANTIKY_ANALYTIC_SOURCE_INDICES[sample];
...
function frameIndexAtTime(timeSeconds: number): number {
  if (!Number.isFinite(timeSeconds)) {
    throw new RangeError("Antiky light time must be finite");
  }
  return Math.max(
    0,
    Math.round(timeSeconds / ANTIKY_FIXED_TIME_STEP_SECONDS),
  );
}

export function createAntikyLightRig(): AntikyLightRig {
  const simulation = createAntikyLightSimulation();
  const particleValues = new Float32Array(ANTIKY_PARTICLE_COUNT * 8);
  const lightValues = new Float32Array(ANTIKY_ANALYTIC_LIGHT_CAPACITY * 8);
  let packedFrame = -1;
...
export function updateAntikyParticleValues(
  values: Float32Array<ArrayBuffer>,
  timeSeconds: number,
): void {
...
export function createAntikyParticleValues(
  timeSeconds = 0,
...
export function updateAntikyLightValues(
  values: Float32Array<ArrayBuffer>,
  timeSeconds: number,
): void {
  const rig = createAntikyLightRig();
...
export function createAntikyLightValues(
  timeSeconds = 0,
): Float32Array<ArrayBuffer> {
  const rig = createAntikyLightRig();
  rig.updateFrame(frameIndexAtTime(timeSeconds));
