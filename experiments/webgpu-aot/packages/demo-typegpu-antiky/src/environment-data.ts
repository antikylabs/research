export interface AntikyHdrImage {
  readonly height: number;
  readonly pixels: Float32Array<ArrayBuffer>;
  readonly width: number;
}

export const ANTIKY_ENVIRONMENT_FACE_URLS = [
  "/textures/px.hdr",
  "/textures/nx.hdr",
  "/textures/py.hdr",
  "/textures/ny.hdr",
...
export function decodeRadianceHdr(source: ArrayBuffer): AntikyHdrImage {
...
export function validateAntikyEnvironmentFaces(
  faces: readonly AntikyHdrImage[],
): number {
  if (faces.length !== 6) {
    throw new Error(
      `Antiky environment requires exactly six cube faces; received ${faces.length}`,
    );
  }
  const first = faces[0];
  if (first === undefined) {
    throw new Error("Antiky environment requires exactly six cube faces");
  }
  ...
  if (
    ...
    throw new Error(
      "Antiky environment cube faces must have matching square dimensions and complete RGBA data",
    );
  }
  return size;
}

export function buildAntikyEnvironmentMipChain(
  source: AntikyHdrImage,
): readonly AntikyHdrImage[] {
  const levels: AntikyHdrImage[] = [source];
  let current = source;
        ...
        for (let channel = 0; channel < 4; channel += 1) {
          ...
          for (let offsetY = 0; offsetY < 2; offsetY += 1) {
            for (let offsetX = 0; offsetX < 2; offsetX += 1) {
              ...
              sum += current.pixels[sourceIndex] ?? 0;
            }
          }
...
export function encodeAntikyRgba16Float(
  pixels: Float32Array,
