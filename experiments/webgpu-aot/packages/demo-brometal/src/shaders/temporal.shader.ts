...
const definition = {
  ...
  bindings: [
    { group: 0, binding: 0, declaration: "var currentHdr: texture_2d<f32>" },
    { group: 0, binding: 1, declaration: "var history: texture_2d<f32>" },
    { group: 0, binding: 2, declaration: "var ambient: texture_2d<f32>" },
    { group: 0, binding: 3, declaration: "var<uniform> settings: TemporalSettings" },
  ],
  functions: [
    {
      attributes: ["@vertex"],
      signature: "temporalVertex(@builtin(vertex_index) index: u32) -> ScreenOut",
      body: `var output: ScreenOut;
  ...
  return output;`,
    ...
    {
      signature: "temporalCurrent(pixel: vec2i) -> vec3f",
      body: `return textureLoad(currentHdr, pixel, 0).rgb *
    textureLoad(ambient, pixel, 0).r;`,
    },
    {
      signature: "temporalClipAabb(value: vec3f, minimum: vec3f, maximum: vec3f) -> vec3f",
      body: `let center = (minimum + maximum) * 0.5;
  ...
  return select(value, center + delta / maximumUnit, maximumUnit > 1.0);`,
    ...
    {
      attributes: ["@fragment"],
      signature: "temporalFragment(input: ScreenOut) -> @location(0) vec4f",
      body: `let size = vec2i(settings.values.xy);
...
export function buildTemporalShader(): string { return compileShader(definition); }
