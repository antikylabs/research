export const BROMETAL_WORKLOAD_SEED = 0x5350_4f4e;
export const BROMETAL_FIXED_TIME_STEP_SECONDS = 1 / 60;
export const BROMETAL_FIRE_LIGHT_COUNT = 256;
export const BROMETAL_CURVE_LIGHT_COUNT = 150;
export const BROMETAL_CORRIDOR_LIGHT_COUNT = 64;
export const BROMETAL_ANIMATED_LIGHT_COUNT =
  BROMETAL_FIRE_LIGHT_COUNT +
  BROMETAL_CURVE_LIGHT_COUNT +
  BROMETAL_CORRIDOR_LIGHT_COUNT;
export const BROMETAL_BILLBOARD_COUNT =
  BROMETAL_FIRE_LIGHT_COUNT + BROMETAL_CURVE_LIGHT_COUNT;

type Vec3 = readonly [number, number, number];

export type BroMetalAnimatedLightKind = "fire" | "curve" | "corridor";

export interface BroMetalAnimatedLightSeed {
  readonly kind: BroMetalAnimatedLightKind;
...
export interface BroMetalAnimatedLightState {
...
export interface BroMetalLightSimulation {
  readonly frameIndex: number;
  readonly seeds: readonly BroMetalAnimatedLightSeed[];
  readonly state: BroMetalAnimatedLightState;
...
export function createBroMetalAnimatedLightSeeds(): readonly BroMetalAnimatedLightSeed[] {
  const random = createMulberry32(BROMETAL_WORKLOAD_SEED);
  const seeds: BroMetalAnimatedLightSeed[] = [];

  for (
    let particle = 0;
    particle < BROMETAL_FIRE_LIGHT_COUNT / 4;
    particle += 1
  ) {
  ...
  for (let index = 0; index < BROMETAL_CURVE_LIGHT_COUNT; index += 1) {
    ...
    seeds.push({
      ...
      velocity,
      color,
      life: index / BROMETAL_CURVE_LIGHT_COUNT,
      lifeSpeed: 0.01,
      lightRadius: 2,
  ...
  for (let side = 0; side < 2; side += 1) {
    for (let index = 0; index < BROMETAL_CORRIDOR_LIGHT_COUNT / 2; index += 1) {
      seeds.push({
        kind: "corridor",
        origin: [0, 0, 0],
        velocity: side === 0 ? [3.125, 1, 0] : [-3.775, -1, 0],
        color: randomColor(random),
        life: index / (BROMETAL_CORRIDOR_LIGHT_COUNT / 2),
        lifeSpeed: 0.01,
        lightRadius: 1,
        billboardRadius: 0,
      });
...
function updateDerivedValues(
  seed: BroMetalAnimatedLightSeed,
  state: BroMetalAnimatedLightState,
  index: number,
): void {
...
function resetState(
  seeds: readonly BroMetalAnimatedLightSeed[],
  state: BroMetalAnimatedLightState,
): void {
...
function stepState(
  ...
  const timeSeconds = frameIndex * BROMETAL_FIXED_TIME_STEP_SECONDS;
  for (let index = 0; index < seeds.length; index += 1) {
    const seed = seeds[index];
    const offset = index * 3;
    let life = state.life[index] +
      seed.lifeSpeed * BROMETAL_FIXED_TIME_STEP_SECONDS;
    if (life >= 1) {
      life = 0;
      state.positions[offset] = seed.origin[0];
      state.positions[offset + 1] = seed.origin[1];
...
export function createBroMetalLightSimulation(): BroMetalLightSimulation {
  const seeds = createBroMetalAnimatedLightSeeds();
  const state: BroMetalAnimatedLightState = {
    life: new Float32Array(BROMETAL_ANIMATED_LIGHT_COUNT),
    positions: new Float32Array(BROMETAL_ANIMATED_LIGHT_COUNT * 3),
    intensities: new Float32Array(BROMETAL_ANIMATED_LIGHT_COUNT),
    lightRadii: new Float32Array(BROMETAL_ANIMATED_LIGHT_COUNT),
    billboardRadii: new Float32Array(BROMETAL_ANIMATED_LIGHT_COUNT),
  };
  let currentFrame = 0;
    ...
    updateFrame(frameIndex: number): void {
      if (!Number.isSafeInteger(frameIndex) || frameIndex < 0) {
        throw new RangeError(
          "BroMetal light frame index must be a non-negative integer",
        );
      }
