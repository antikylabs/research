...
import {
  BROMETAL_ANALYTIC_SOURCE_INDICES,
...
function referenceStateAtFrame(frame: number) {
  ...
  for (let index = 1; index <= frame; index += 1) {
    stepAnimatedLights(
      seeds,
      state,
      index * BROMETAL_FIXED_TIME_STEP_SECONDS,
      BROMETAL_FIXED_TIME_STEP_SECONDS,
    );
  }
  return { seeds, state };
...
describe("BroMetal deterministic lights", () => {
  it.each([0, 12, 828])(
  ...
  it("uses the Three-equivalent main fire parameters", () => {
    const lights = createBroMetalLightValues(0);
  ...
  it("matches authoritative analytic fire records at frame 12", () => {
    createBroMetalLightValues(10);
    const lights = createBroMetalLightValues(12 / 60);
  ...
  it.each([0, 5, 10])(
    ...
    (time) => {
      const particles = createBroMetalParticleValues(time);
      const lights = createBroMetalLightValues(time);
      const sampledX: number[] = [];

      for (const [
        sampleIndex,
        sourceIndex,
      ] of CURVE_LIGHT_SOURCE_INDICES.entries()) {
  ...
  it("matches authoritative packed curve records at frame 12", () => {
    createBroMetalParticleValues(10);
    const particles = createBroMetalParticleValues(12 / 60);
  ...
  it.each([0, 12])(
    ...
    (frame) => {
      ...
      for (let index = 0; index < 406; index += 1) {
        ...
        expect(rig.particleValues.slice(particleOffset, particleOffset + 3))
        ...
        expect(rig.particleValues.slice(particleOffset + 4, particleOffset + 8))
  ...
  it.each([0, 12])(
    ...
    (frame) => {
      ...
      for (const [sample, sourceIndex] of BROMETAL_ANALYTIC_SOURCE_INDICES.entries()) {
        ...
        expect(rig.lightValues.slice(valueOffset, valueOffset + 8)).toEqual(expected);
      }
    },
  );

  it("updates stable buffers once per frame and resets after a rewind", () => {
    const direct = createBroMetalLightRig();
    const particles = direct.particleValues;
    const lights = direct.lightValues;
    direct.updateFrame(12);
    const frame12Particles = particles.slice();
    const frame12Lights = lights.slice();
    expect(direct.particleValues).toBe(particles);
    expect(direct.lightValues).toBe(lights);

    direct.updateFrame(12);
    expect(particles).toEqual(frame12Particles);
    expect(lights).toEqual(frame12Lights);
    direct.updateFrame(600);
    direct.updateFrame(12);
    expect(particles).toEqual(frame12Particles);
    expect(lights).toEqual(frame12Lights);

    const sequential = createBroMetalLightRig();
