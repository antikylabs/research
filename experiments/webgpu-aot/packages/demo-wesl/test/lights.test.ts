...
import { createWeslLightSimulation } from "../src/light-state.js";
...
function referenceStateAtFrame(frame: number) {
  ...
  for (let index = 1; index <= frame; index += 1) {
    stepAnimatedLights(seeds, state, index / 60, 1 / 60);
  }
  return { seeds, state };
}

describe("WESL deterministic lights", () => {
  it.each([0, 12])(
  ...
  it("uses the reference main-fire parameters", () => {
    const lights = createWeslLightValues(0);
  ...
  it("matches authoritative analytic-fire records at frame 12", () => {
    createWeslLightValues(10);
    const lights = createWeslLightValues(12 / 60);
  ...
  it.each([0, 5, 10])(
    ...
    (time) => {
      const particles = createWeslParticleValues(time);
      const lights = createWeslLightValues(time);
  ...
  it("matches authoritative packed curve records at frame 12", () => {
    createWeslParticleValues(10);
    const particles = createWeslParticleValues(12 / 60);
    ...
    (frame) => {
      ...
      for (let index = 0; index < 406; index += 1) {
        ...
        expect(
          ...
          Array.from(
            state.positions.slice(positionOffset, positionOffset + 3),
          ),
        );
        ...
        ).toEqual(Array.from(new Float32Array([...seeds[index].color, 1])));
      }
    },
  );
  ...
  it.each([0, 12])(
    ...
    (frame) => {
      ...
      for (const [sample, sourceIndex] of WESL_ANALYTIC_SOURCE_INDICES.entries()) {
  ...
  it("updates once per frame and resets deterministically after a rewind", () => {
    const direct = createWeslLightRig();
    ...
    const sequential = createWeslLightRig();
