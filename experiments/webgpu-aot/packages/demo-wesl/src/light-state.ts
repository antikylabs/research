export const WESL_WORKLOAD_SEED = 0x5350_4f4e;
export const WESL_FIXED_TIME_STEP_SECONDS = 1 / 60;
export const WESL_FIRE_LIGHT_COUNT = 256;
export const WESL_CURVE_LIGHT_COUNT = 150;
export const WESL_CORRIDOR_LIGHT_COUNT = 64;
export const WESL_ANIMATED_LIGHT_COUNT =
  WESL_FIRE_LIGHT_COUNT +
  WESL_CURVE_LIGHT_COUNT +
  WESL_CORRIDOR_LIGHT_COUNT;
export const WESL_BILLBOARD_COUNT =
  WESL_FIRE_LIGHT_COUNT + WESL_CURVE_LIGHT_COUNT;

type Vec3 = readonly [number, number, number];

export type WeslAnimatedLightKind = "fire" | "curve" | "corridor";

export interface WeslAnimatedLightSeed {
  readonly kind: WeslAnimatedLightKind;
...
export interface WeslAnimatedLightState {
...
export interface WeslLightSimulation {
  readonly frameIndex: number;
  readonly seeds: readonly WeslAnimatedLightSeed[];
  readonly state: WeslAnimatedLightState;
...
export function createWeslAnimatedLightSeeds(): readonly WeslAnimatedLightSeed[] {
  const random = createMulberry32(WESL_WORKLOAD_SEED);
  const seeds: WeslAnimatedLightSeed[] = [];

  for (let particle = 0; particle < WESL_FIRE_LIGHT_COUNT / 4; particle += 1) {
  ...
  for (let index = 0; index < WESL_CURVE_LIGHT_COUNT; index += 1) {
    ...
    seeds.push({
      ...
      velocity,
      color,
      life: index / WESL_CURVE_LIGHT_COUNT,
      lifeSpeed: 0.01,
      lightRadius: 2,
  ...
  for (let side = 0; side < 2; side += 1) {
    for (let index = 0; index < WESL_CORRIDOR_LIGHT_COUNT / 2; index += 1) {
      seeds.push({
        kind: "corridor",
        origin: [0, 0, 0],
        velocity: side === 0 ? [3.125, 1, 0] : [-3.775, -1, 0],
        color: randomColor(random),
        life: index / (WESL_CORRIDOR_LIGHT_COUNT / 2),
        lifeSpeed: 0.01,
        lightRadius: 1,
        billboardRadius: 0,
      });
...
function updateDerivedValues(
  seed: WeslAnimatedLightSeed,
  state: WeslAnimatedLightState,
  index: number,
): void {
...
function resetState(
  seeds: readonly WeslAnimatedLightSeed[],
  state: WeslAnimatedLightState,
): void {
...
function stepState(
  ...
  const timeSeconds = frameIndex * WESL_FIXED_TIME_STEP_SECONDS;
  for (let index = 0; index < seeds.length; index += 1) {
    const seed = seeds[index];
    const offset = index * 3;
    let life = state.life[index] +
      seed.lifeSpeed * WESL_FIXED_TIME_STEP_SECONDS;
    if (life >= 1) {
      life = 0;
      state.positions[offset] = seed.origin[0];
      state.positions[offset + 1] = seed.origin[1];
...
export function createWeslLightSimulation(): WeslLightSimulation {
  const seeds = createWeslAnimatedLightSeeds();
  const state: WeslAnimatedLightState = {
    life: new Float32Array(WESL_ANIMATED_LIGHT_COUNT),
    positions: new Float32Array(WESL_ANIMATED_LIGHT_COUNT * 3),
    intensities: new Float32Array(WESL_ANIMATED_LIGHT_COUNT),
    lightRadii: new Float32Array(WESL_ANIMATED_LIGHT_COUNT),
    billboardRadii: new Float32Array(WESL_ANIMATED_LIGHT_COUNT),
  };
  let currentFrame = 0;
    ...
    updateFrame(frameIndex: number): void {
      if (!Number.isSafeInteger(frameIndex) || frameIndex < 0) {
        throw new RangeError(
          "WESL light frame index must be a non-negative integer",
        );
      }
