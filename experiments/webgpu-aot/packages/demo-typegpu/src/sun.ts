...
export type TypeGpuVector3 = readonly [number, number, number];

const normalize = (value: TypeGpuVector3): TypeGpuVector3 => {
...
export const TYPEGPU_SHADOW_CASCADE_COUNT =
  TYPEGPU_SUN.shadow.cascadeCount;
export const TYPEGPU_SHADOW_RESOLUTION = TYPEGPU_SUN.shadow.resolution;

export interface TypeGpuSunCascadePlan {
  readonly bias: number;
  readonly farDistance: number;
  readonly frustumCorners: readonly TypeGpuVector3[];
  readonly nearDistance: number;
  readonly orthographicSize: number;
  readonly viewProjection: Float32Array;
}

export interface TypeGpuSunPlan {
  readonly cameraForward: TypeGpuVector3;
  readonly cascades: readonly [TypeGpuSunCascadePlan, TypeGpuSunCascadePlan];
  readonly direction: TypeGpuVector3;
...
const addScaled = (
  origin: TypeGpuVector3,
  direction: TypeGpuVector3,
  scale: number,
): TypeGpuVector3 => [
...
const dot = (left: TypeGpuVector3, right: TypeGpuVector3): number =>
  left[0] * right[0] + left[1] * right[1] + left[2] * right[2];

const distance = (left: TypeGpuVector3, right: TypeGpuVector3): number =>
...
function frustumSliceCorners(
  ...
  nearDistance: number,
  farDistance: number,
  cameraForward: TypeGpuVector3,
): readonly TypeGpuVector3[] {
  const cameraRight = normalize(cross(cameraForward, TYPEGPU_CAMERA.up));
  const cameraUp = normalize(cross(cameraRight, cameraForward));
  const tangent = Math.tan(TYPEGPU_CAMERA.fieldOfViewRadians / 2);
  const planeCorners = (
...
function fitCascade(
  ...
  const lightZ = TYPEGPU_SUN.direction;
  const lightX = normalize(cross(TYPEGPU_CAMERA.up, lightZ));
  ...
  const cameraRange = TYPEGPU_CAMERA.far - TYPEGPU_CAMERA.near;
  ...
  const texelSize = orthographicSize / TYPEGPU_SHADOW_RESOLUTION;
  const centerX =
  ...
  const eyeZ = maxZ + TYPEGPU_SUN.shadow.lightMargin;
  const eye: TypeGpuVector3 = [
  ...
  const view = mat4.lookAt(eye, target, TYPEGPU_CAMERA.up);
  const projection = mat4.ortho(
    ...
    orthographicSize / 2,
    0.1,
    maxZ - minZ + TYPEGPU_SUN.shadow.lightMargin * 2,
  );
...
export function createTypeGpuSunPlan(aspect: number): TypeGpuSunPlan {
  if (!Number.isFinite(aspect) || aspect <= 0) {
    throw new Error("TypeGPU sun plan requires a positive finite aspect");
  }
  const { far, near, position, target } = TYPEGPU_CAMERA;
  ...
  return {
    ...
    direction: TYPEGPU_SUN.direction,
    fadeEndDistance,
    fadeStartDistance,
    resolution: TYPEGPU_SHADOW_RESOLUTION,
    splitDistance,
    splitNormalized,
  };
}
