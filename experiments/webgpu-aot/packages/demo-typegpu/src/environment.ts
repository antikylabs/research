...
export interface TypeGpuEnvironment {
...
export function buildEnvironmentMipChain(source: HdrImage): readonly HdrImage[] {
  const levels: HdrImage[] = [source];
  let current = source;
...
export async function loadTypeGpuEnvironment(
  device: GPUDevice,
  display: LoadingDisplay,
): Promise<TypeGpuEnvironment> {
  display.update({
    done: 4,
    message: "Decoding six native HDR environment faces",
    total: 8,
  });
  const faces = await Promise.all(
    ENVIRONMENT_FACE_URLS.map(async (url) =>
  ...
  const mipChains = faces.map(buildEnvironmentMipChain);
